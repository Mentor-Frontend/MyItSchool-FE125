...
export default function Salad(props) {
  ...
  return (
    ...
    >
      ...
      <MainCounterStyled
        style={{ marginTop: "8px", alignItems: "center", marginBottom: "16px" }}
      >
        <PlusMinusStyled
        ...
        <NumberStyled style={{ color: props.product.color }}>
          {count}
        </NumberStyled>
        <PlusMinusStyled
          style={{ width: "24px", height: "24px", opacity: count === 50 ? '0.3' : '1' }}
          onClick={handlePlus}
      ...
      <BlockInfoStyled>
        ...
        <PriceStyled>
          <CostTextStyled style={{ color: props.product.color }}>
            9.50
          </CostTextStyled>
          <BynTextStyled style={{ color: props.product.color }}>
            BYN
          </BynTextStyled>
        </PriceStyled>
      </BlockInfoStyled>
      <ButtonStyled ref={target}
        onClick={() => {
          ...
          document.cookie = `product1=${JSON.stringify(
            countCookie
          )}; max-age=${age}`;
          editCart();
        }}
      ...
      <Overlay target={target.current} show={show} placement={isBottom <= 441 ? "bottom" : "left"} delay={{ show: 250, hide: 400 }}>
        {(props) => (
          <Tooltip className={isBottom <= 441 ? "tooltip tooltip-bottom" : "tooltip tooltip-right"} {...props}>
